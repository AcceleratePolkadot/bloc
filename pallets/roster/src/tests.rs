use crate::{mock::*, Error, Event, Rosters, types};
use frame_support::{pallet_prelude::*, assert_noop, assert_ok,};
...
fn test_can_create_new_roster() {
    new_test_ext().execute_with(|| {
        ...
        let bounded_title_result: Result<types::RosterTitle<Test>, Error<Test>> = roster_title.try_into().map_err(|_| Error::<Test>::InvalidRosterTitle);
        ...
        // Create Roster
        assert_ok!(Roster::roster_new(RuntimeOrigin::signed(ALICE), "My Roster".as_bytes().to_vec()));
...
fn test_roster_title_unique_to_account() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_last_event(Event::NewRoster { founder: ALICE, title: bounded_title.clone(), roster_id: alice_roster_id }.into());

        // Bob creates a roster with the same title
        assert_ok!(Roster::roster_new(RuntimeOrigin::signed(BOB), title.clone()));
        System::assert_last_event(Event::NewRoster { founder: BOB, title: bounded_title.clone(), roster_id: bob_roster_id }.into());
...
fn test_can_remove_inactive_roster() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(Event::RosterRemoved {
            removed_by: ALICE,
            roster_id: roster_id.clone()
        }.into());
    });
