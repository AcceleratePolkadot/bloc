...
pub type NominationsList<T> =
	BoundedVec<<T as Config>::AccountId, <T as pallet::Config>::NominationsPerRosterMax>;
pub type ExpulsionProposalsList<T> = BoundedVec<
	(<T as Config>::AccountId, <T as Config>::AccountId),
	<T as pallet::Config>::ExpulsionProposalsPerRosterMax,
>;
...
impl RosterId {
	pub fn from_tuple<T: Config + pallet::Config>(
		(founder, title): (&T::AccountId, &RosterTitle<T>),
	) -> Self {
	...
	pub fn from_tuple_with_unbounded_title<T: Config>(
		(founder, title): (&T::AccountId, &Vec<u8>),
	) -> Self {
		let namespace = Uuid::from_bytes(md5::compute(founder.encode()).0);
		let roster_uuid = Uuid::new_v3(&namespace, &title);
		RosterId(*roster_uuid.as_bytes())
...
impl<T: pallet::Config> Roster<T> {
	pub fn new(founder: &T::AccountId, title: &RosterTitle<T>) -> Self {
		Self {
...
impl<T: Config + pallet::Config> Nomination<T> {
	pub fn new(roster: &RosterId, nominee: &T::AccountId, nominator: &T::AccountId) -> Self {
		Self {
			roster: roster.clone(),
...
impl<T: Config> NominationVote<T> {
	pub fn new(voter: &T::AccountId, vote: &NominationVoteValue) -> Self {
		Self {
			voter: voter.clone(),
			vote: vote.clone(),
...
pub type NominationVotes<T> =
	BoundedVec<NominationVote<T>, <T as pallet::Config>::NominationVotesMax>;
...
impl<T: Config + pallet::Config> ExpulsionProposalVote<T> {
	pub fn new(voter: &T::AccountId, vote: &ExpulsionProposalVoteValue) -> Self {
		Self {
			voter: voter.clone(),
			vote: vote.clone(),
			voted_on: <system::Pallet<T>>::block_number(),
...
pub type ExpulsionProposalVotes<T> =
	BoundedVec<ExpulsionProposalVote<T>, <T as pallet::Config>::ExpulsionProposalVotesMax>;
